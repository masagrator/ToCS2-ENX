...
def AnimeClipTable(entry):
	ret_entry = []
	match(entry["TYPE"]):
		case "0x0":
			if entry["SUBTYPE"] == "0x0":
				ret_entry.append(numpy.uint32(0))
				ret_entry.append(bytes.fromhex(entry["DATA"]))
			else:
				ret_entry.append(numpy.uint8(0))
				ret_entry.append(numpy.uint8(int(entry["SUBTYPE"], base=16)))
				ret_entry.append(numpy.uint16(0))
				ret_entry.append(entry["STRINGS"][0].encode("UTF-8") + b"\x00")
				for i in range(0, 32 - len(entry["STRINGS"][0].encode("UTF-8") + b"\x00")):
					ret_entry.append(numpy.uint8(0))
				ret_entry.append(entry["STRINGS"][1].encode("UTF-8") + b"\x00")
				for i in range(0, 32 - len(entry["STRINGS"][1].encode("UTF-8") + b"\x00")):
					ret_entry.append(numpy.uint8(0))
		case _:
			ret_entry.append(numpy.uint32(int(entry["TYPE"], base=16)))
			ret_entry.append(entry["STRINGS"][0].encode("UTF-8") + b"\x00")
			for i in range(0, 32 - len(entry["STRINGS"][0].encode("UTF-8") + b"\x00")):
				ret_entry.append(numpy.uint8(0))
			ret_entry.append(entry["STRINGS"][1].encode("UTF-8") + b"\x00")
...
def GenerateCommand(entry, Offset_dict = None):
	...
	match(entry["TYPE"]):
		...
		case "0x4A":
			...
			ret_entry.append(numpy.uint8(entry["CONTROL"]))
			match(entry["CONTROL"]):
				case 0x2E | 0x36 | 0x43 | 0x41 | 0x12 | 0x2D | 0x10 | 0x11 | 0x2F | 0x13 | 0x1E | 0x31 | 0x2A | 0x25 | 0xF | 0x1D | 0x3F | 0x27 | 0x1F | 0x1B | 0x1A:
					pass
...
files = glob.glob("scripts/ani/jsons/npc*.json")
...
for i in range(0, len(files)):
	...
	# Write Header
	file_new = open("new_nx/%s.dat" % files[i][18:-5], "wb")
	...
	for x in range(0, len(keys)):
		...
		for y in range(0, len(DUMP["FUNCTIONS"][keys[x]])):
			...
			if (keys[x] in ["AnimeClipTable", "FieldMonsterData", "FieldFollowData"]):
				match(keys[x]):
					case "AnimeClipTable":
						offset += len(AnimeClipTable(entry))
					case _:
						offset += len(bytes.fromhex(entry))
				continue
		...
		for y in range(0, len(DUMP["FUNCTIONS"][keys[x]])):
			...
			match(keys[x]):
				case "AnimeClipTable":
					COMMAND_BLOCK.append(AnimeClipTable(entry))
				case "FieldMonsterData" | "FieldFollowData":
					COMMAND_BLOCK.append(bytes.fromhex(entry))
				case _:
					if (keys[x][0:1] == "_"):
